...
parser.add_argument('--lr_cubic', default=0.0001, type=float, help='learning rate for cubic')
...
def cubic_approx(net, epoch, optimizer, criterion, inputs_grad, targets_grad, inputs_hv, targets_hv):
    ...
    # lr_cubic = args.lr_cubic * (0.5 ** (epoch // 50))
    lr_cubic = args.lr_cubic

    net_delta_optimizer = torch.optim.SGD(net_delta.parameters(), lr=lr_cubic, momentum=args.cubic_momentum, weight_decay=0.0)
    ...
    for p in net_delta.parameters():
        p.data /= (- 1.0 * np.sqrt(norm_grad_cubic) / args.rc)
    ...
    for epoch_c in range(args.cubic_epoch):
        ...
        # clip H*v
        torch.nn.utils.clip_grad_norm(net_delta.parameters(), 1.0)
    ...
    for param_net, param_delta in zip(net.parameters(), net_delta.parameters()):
        param_net.grad.data = - 1.0 * param_delta.data * 1e6

    # norm_net = 0.0
    # for p_net in net.parameters():
    #     norm_delta += p_net.grad.data.norm(2) ** 2
    # norm_net = np.sqrt(norm_delta)
    # print("norm_net: ", norm_net)
