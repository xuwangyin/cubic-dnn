...
from mnist_f1 import mnist_model
import numpy as np
import copy
...
def cubic_approx(net, epoch, optimizer, criterion, inputs_grad, targets_grad, inputs_hv, targets_hv):
    ...
    # adjust_learning_rate_cubic(net_delta_optimizer, epoch)
    ...
    for p_delta in net_delta.parameters():
        norm_delta += p_delta.data.norm(2) ** 2
    norm_delta = np.sqrt(norm_delta)
    # print("norm_net_delta_grad, 0, after ", norm_delta)
    ...
    for epoch_c in range(args.cubic_epoch):
        ...
        # print("norm_net_delta_grad, 1, after ", norm_net_delta_grad)
        ...
        for p in net_delta.parameters():
            norm_net_delta_grad += p.grad.data.norm(2) ** 2
        norm_net_delta_grad = float(np.sqrt(norm_net_delta_grad))
        # print("norm_net_delta_grad, 2, before  ", norm_net_delta_grad)

        # clipping the gradient
        # clip_gradient(net_delta_optimizer, 0.005)
        ...
        for p in net_delta.parameters():
            norm_net_delta_grad += p.grad.data.norm(2) ** 2
        norm_net_delta_grad = float(np.sqrt(norm_net_delta_grad))
        # print("norm_net_delta_grad, 2, after  ", norm_net_delta_grad)

        norm_net_grad = 0.0
        for p in net_delta.parameters():
            norm_net_grad += p.data.norm(2) ** 2
        norm_net_grad = float(np.sqrt(norm_net_grad))
        # print("norm_net_grad, 3, during, 1 ", norm_net_grad)
        ...
        for p in net_delta.parameters():
            norm_net_grad += p.data.norm(2) ** 2
        norm_net_grad = float(np.sqrt(norm_net_grad))
        # print("norm_net_grad, 3, during, 2  ", norm_net_grad)
    ...
    for p in net_delta.parameters():
        norm_net_grad += p.data.norm(2) ** 2
    norm_net_grad = float(np.sqrt(norm_net_grad))
    # print("norm_net_grad, 3, before  ", norm_net_grad)
    ...
    for p in net.parameters():
        norm_net_grad += p.grad.data.norm(2) ** 2
    norm_net_grad = float(np.sqrt(norm_net_grad))
    # print("norm_net_grad, 3, after  ", norm_net_grad)
...
def adjust_learning_rate_cubic(optimizer, epoch):
    """Sets the learning rate to the initial LR decayed by 2 every 30 epochs"""
    lr_cubic = args.lr_cubic * (0.1 ** (epoch // 50))
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr_cubic
